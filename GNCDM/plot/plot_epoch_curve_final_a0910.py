# -*- coding: utf-8 -*-
"""Merge a0910 epoch CSVs; plot ACC_old | ACC_new as one 1x2 figure (old left).

  cd GNCDM/plot && python plot_epoch_curve_final_a0910.py --epochs 25
"""
...
def paths(n_epoch):
    tag = f"ep{n_epoch}"
    return {
        "gncdm": os.path.join(SAVE_DIR, f"epoch_curve_gncdm_a0910_random_split_{tag}.csv"),
        "aval": os.path.join(SAVE_DIR, f"epoch_curve_avalanche_a0910_random_split_{tag}.csv"),
        "out_csv": os.path.join(SAVE_DIR, f"epoch_curve_a0910_random_split_final_{tag}.csv"),
        "out_png": os.path.join(SAVE_DIR, f"epoch_curve_a0910_random_split_final_{tag}.png"),
    }
...
def main():
    ...
    # ponytail: paper layout — old left, new right
    draw_panel(
        axes[0],
        df,
        n,
