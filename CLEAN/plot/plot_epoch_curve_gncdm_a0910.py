# -*- coding: utf-8 -*-
"""a0910 random_split ACC_new/ACC_old epoch curves (GNCDM backbone). Mirrors math1.

Run on GPU server:
  cd CLEAN/plot && python plot_epoch_curve_gncdm_a0910.py --epochs 25
"""
...
def load_ctx(device, n_epoch):
    ...
    print(
        f"a0910 dims n_user={n_user} items={n_item_old}+{n_item_new} "
...
def run_xder_curve(curve_eval_fn, device, n_epoch, meta):
    history = []
    row = run_xder(
        split_name="a0910_random_split(curve)",
        ds_name="a0910_curve",
...
def run_clora_gncdm_curve(device, n_epoch):
    cfg = CL._config_for_dataset("a0910")
    cfg = dict(cfg)
    cfg["name"] = "a0910"
...
def main():
    ...
    out = os.path.join(SAVE_DIR, f"epoch_curve_gncdm_a0910_random_split_ep{n_epoch}.csv")
