# -*- coding: utf-8 -*-
"""a0910 random_split EWC/DER++ epoch curves. Needs avalanche.

  cd CLEAN/plot && python plot_epoch_curve_avalanche_a0910.py --epochs 25
"""
...
EWC_LAMBDA = 10000  # all_methods_a0910_random_split.csv EWC (lambda=10000)
...
def main():
    ...
    cfg = {
        ...
        "n_item": int(__import__("numpy").load(os.path.join(a0910, "Q_matrix.npy")).shape[0]),
        "n_know": int(__import__("numpy").load(os.path.join(a0910, "Q_matrix.npy")).shape[1]),
        "new_concepts": "auto",
    }
    meta = CB.load_random(cfg)
    rows = ewc_curve(meta, device, args.epochs)
    rows += der_curve(meta, device, args.epochs)
    out = os.path.join(SAVE_DIR, f"epoch_curve_avalanche_a0910_random_split_ep{args.epochs}.csv")
