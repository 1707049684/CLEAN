...
`experiments/` 只放增量学习主实验入口（`run_incremental_*`）与 `_core/` 管线库；画图脚本依赖实验库时，在脚本头部把 `CLEAN/`、`experiments/`、`experiments/_core/` 加入 `sys.path`。
...
产物 CSV/PNG 仍写入 `CLEAN/incremental_result/`（与主实验结果同目录）。
